...
class VideoPlayer : NSObject {
    ...
    convenience init(urlAsset:NSURL, view:PlayerView, arr:[Double], startAutoPlay:Bool = true, repeatAfterEnd:Bool = false) {
        self.init()
    ...
    private func pauseCheck(time: Double) {
        for reference in pauseArray! {
            if (reference + 0.04) > time && (reference - 0.04) < time {
                pause()
                videoDelegate().endRelay()
            }
        }
    }
